<script lang="ts">
	...
	import DataCollectionForm from '$components/legal-gen/DataCollectionForm.svelte';
	...
	function handleDownloadMarkdown() {
		downloadMarkdown(markdown, 'privacy-policy');
	}

	function handleDownloadHtml() {
		downloadHtml(markdown, 'privacy-policy');
	}
...
<svelte:head>
	<title>Privacy Policy Generator — nah</title>
	<meta name="description" content="Generate a professional privacy policy that complies with GDPR, CCPA, and other regulations." />
</svelte:head>
...
<div class="min-h-screen bg-surface">
	...
	<div class="bg-surface-alt border-b border-border">
		<div class="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
			...
			<h1 class="text-3xl font-bold text-text font-display">Privacy Policy Generator</h1>
			<p class="text-text-muted mt-2">Customize your policy and download in multiple formats</p>
		</div>
	</div>
	...
	<div class="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
		<div class="grid lg:grid-cols-2 gap-8">
			...
			<div class="space-y-8">
				...
				<div class="bg-surface border border-border rounded-xl p-6">
					<h2 class="text-lg font-semibold text-text mb-4 font-display">Data Collection</h2>
					<DataCollectionForm bind:config={input.dataCollection} />
				</div>
