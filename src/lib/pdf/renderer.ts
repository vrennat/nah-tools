...
export async function pdfToImages(
	...
	// try/finally ensures the PDF.js doc is destroyed even on mid-loop errors
	try {
		...
		// Release canvas backing store before the doc, so the GPU texture is freed
		canvas.width = 0;
		canvas.height = 0;
		doc.destroy();
	}
...
export async function extractImages(
	...
	try {
		...
		// Release canvas backing store before destroying the doc
		canvas.width = 0;
		canvas.height = 0;
		doc.destroy();
	}
