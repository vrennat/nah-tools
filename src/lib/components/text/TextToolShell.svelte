<script lang="ts">
	...
	import { textTools, type TextTool } from '$text/tools';
	...
	const canonical = $derived(`https://nah.tools/text/${slug}`);
	const related = $derived(textTools.filter((t: TextTool) => t.slug !== slug));
</script>
...
<div class="space-y-8 py-2">
	<nav class="flex items-center gap-1.5 text-sm text-text-muted" aria-label="Breadcrumb">
		...
		<a href="/text" class="transition-colors hover:text-accent">Text Tools</a>
		<span aria-hidden="true">/</span>
	...
	<section class="space-y-4 border-t border-border pt-8">
		<h2 class="font-display text-lg font-700">More text tools</h2>
