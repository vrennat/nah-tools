	...
	<div class="flex items-center justify-between">
		...
		<div class="flex items-center">
			...
			<nav class="hidden items-center text-[15px] sm:flex">
				...
				<!-- Command palette trigger — visible on desktop only -->
				<button
					onclick={(e: MouseEvent) => { e.stopPropagation(); document.dispatchEvent(new KeyboardEvent('keydown', { key: 'k', metaKey: true, bubbles: true })); }}
					class="hidden items-center gap-1.5 rounded-md border border-border px-2.5 py-1 text-[13px] text-text-muted transition-colors hover:border-accent/50 hover:text-accent sm:flex"
