...
<div class="rounded-xl border border-border bg-surface p-6 shadow-sm">
	...
	<div class="grid gap-4 sm:grid-cols-2">
		<div>
			...
			<input id="first-name" type="text" bind:value={userInfo.firstName} placeholder="Jane" class={inputClass} />
		</div>
		<div>
			<label for="last-name" class="mb-1 block text-sm font-medium text-text">Last name</label>
			<input id="last-name" type="text" bind:value={userInfo.lastName} placeholder="Doe" class={inputClass} />
		</div>
		<div>
			<label for="email" class="mb-1 block text-sm font-medium text-text">Email</label>
			<input id="email" type="email" bind:value={userInfo.email} placeholder="jane@example.com" class={inputClass} />
		</div>
		<div>
			<label for="state" class="mb-1 block text-sm font-medium text-text">State</label>
			<select id="state" bind:value={userInfo.state} class={inputClass}>
	...
	<details class="group mt-4" bind:open={showExtra}>
		...
		<div class="mt-3 grid gap-4 sm:grid-cols-2">
			<div class="sm:col-span-2">
				...
				<input id="address" type="text" bind:value={userInfo.address} placeholder="123 Main St" class={inputClass} />
			</div>
			<div>
				<label for="city" class="mb-1 block text-sm font-medium text-text">City</label>
				<input id="city" type="text" bind:value={userInfo.city} placeholder="San Francisco" class={inputClass} />
			</div>
			<div>
				<label for="zip" class="mb-1 block text-sm font-medium text-text">ZIP code</label>
				<input id="zip" type="text" bind:value={userInfo.zip} placeholder="94102" class={inputClass} />
			</div>
			<div>
				<label for="phone" class="mb-1 block text-sm font-medium text-text">Phone</label>
				<input id="phone" type="tel" bind:value={userInfo.phone} placeholder="(555) 123-4567" class={inputClass} />
			</div>
		</div>
	</details>
</div>
